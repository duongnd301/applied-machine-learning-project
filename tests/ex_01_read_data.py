...
def create_sliding_windows_first_dim(data: np.ndarray, sequence_length: int) -> np.ndarray:
    ...
    # sliding_window_view returns shape (n_windows, timesteps, features, sequence_length).
    windows = np.lib.stride_tricks.sliding_window_view(data, window_shape=sequence_length, axis=0)

    # Move sequence_length before timesteps/features:
    # (n_windows, timesteps, features, sequence_length)
